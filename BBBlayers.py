...
class Reshape(nn.Module):
    ...
    def __init__(self, num_features, w):
        super(Reshape, self).__init__()
        self.num_features = num_features
        self.width = w

    def forward(self, x):
        return x.view(-1, self.num_features, self.width, self.width)
...
class _ConvNd(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
                 padding, dilation, transposed, output_padding, groups, p_logvar_init=-3, p_pi= 1.0, q_logvar_init=-5):
        ...
        if transposed:
            ...
            self.qw_mean = Parameter(torch.Tensor(in_channels, out_channels // groups, *kernel_size))
            self.qw_logvar = Parameter(torch.Tensor(in_channels, out_channels // groups, *kernel_size))
            # self.qb_mean = Parameter(torch.Tensor(out_channels))
            # self.qb_logvar = Parameter(torch.Tensor(out_channels))

            # ...and output...
            self.conv_qw_mean = Parameter(torch.Tensor(in_channels, out_channels // groups, *kernel_size))
            self.conv_qw_si = Parameter(torch.Tensor(in_channels, out_channels // groups, *kernel_size))
...
class _ConvTransposeMixin(object):
    ...
    def forward(self, input, output_size=None):
        ...
        if self.bias is None:
            return func(input, self.qw_mean)
        else:
            return func(input, self.qw_mean, self.bias)
...
class BBBTranspose2d(_ConvTransposeMixin, _ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        super(BBBTranspose2d, self).__init__(
    ...
    def convprobforward(self, input):
        """
        TransposeConvolutional probabilistic forwarding method.
        :param input: data tensor
        :return: output, KL-divergence
        """

        # local reparameterization trick for convolutional layer
        conv_qw_mean = F.conv_transpose2d(input = input, weight = self.qw_mean, stride = self.stride, padding = self.padding,
            output_padding = self.output_padding, groups =self.groups, dilation = self.dilation)
        conv_qw_si = torch.sqrt(1e-8 + F.conv_transpose2d(input=input.pow(2), weight=torch.exp(self.log_alpha)*self.qw_mean.pow(2),
...
class BBBConv2d(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        super(BBBConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, False, _pair(0), groups)
